"""v0.8.4-I readiness check: Roadmap Amendment / Final Closeout Addendum.

Pure local filesystem + git metadata validation, standard library only. It reads the
v0.8.4-I doc and this script's own source directly from the working tree, and checks that
no existing tracked file was modified by this round.

Like v0.8.4-H, v0.8.4-I is a roadmap amendment / final closeout addendum with no builder
and no fixture, so this script does *not* import any application module, does not import
any earlier builder module, does not re-run any other script as a subprocess, and does not
touch the queue, secrets, or the network. It is file content / path / marker validation
only. Its only subprocess use is read-only git plumbing (rev-parse, status, diff,
ls-files, tag, merge-base).
"""
...
I_DOC_PATH = REPO_ROOT / I_DOC_REL

I_SCRIPT_REL = (
    "scripts/check_hermes_openclaw_roadmap_amendment_final_closeout_addendum_v0_8_4_i.py"
)
I_SCRIPT_PATH = REPO_ROOT / I_SCRIPT_REL

# -----------------------------------------------------------------------------------
# v0.8.4-A..H (this series)
# -----------------------------------------------------------------------------------
...
# v0.8.4-H commit — HEAD at the start of this round.
EXPECTED_BASE_HEAD = "c1a424d7cf292886b87528fa97ae3e47767b61d5"
EXPECTED_COMMIT_MESSAGE = "docs: add owner review decision boundary addendum"

REQUIRED_TEXT_MARKERS = (
    "v0.8.4-I",
    "Roadmap Amendment / Final Closeout Addendum",
    "v0.8.4-H = DONE / PUSHED / CLOSED",
    EXPECTED_BASE_HEAD,
    "v0.8.5 is reserved for the OpenClaw Mock Gateway",
    "Owner Review Decision Boundary does not occupy v0.8.5",
...
UNSAFE_DONE_CLAIM_PARTS = (
    ...
    ("command envelope", " created"),
    ("mock gateway helper", " created"),
    ("v0.8.5", " started"),
    ("v0.8.5-A", " started"),
    ("OpenClaw Mock Gateway", " started"),
)
...
def detect_phase() -> str:
    i_tracked = is_tracked(I_SCRIPT_REL)
    if not i_tracked:
        return "owner_review"
...
def main() -> None:
    doc_text = read_text(I_DOC_PATH)
    self_text = read_text(I_SCRIPT_PATH)
    ...
    print("[B] v0.8.4-I doc exists")
    check("B. v0.8.4-I doc exists", I_DOC_PATH.is_file())

    print("[C] v0.8.4-I readiness script exists")
    check("C. v0.8.4-I readiness script exists", I_SCRIPT_PATH.is_file())

    print("[D] doc is untracked in Owner Review phase")
    check(
        "D. doc is untracked in Owner Review phase",
        phase != "owner_review" or not is_tracked(I_DOC_REL),
    )

    print("[E] readiness script is untracked in Owner Review phase")
    check(
        "E. readiness script is untracked in Owner Review phase",
        phase != "owner_review" or not is_tracked(I_SCRIPT_REL),
    )
    ...
    print("[G] untracked only v0.8.4-I doc, v0.8.4-I script, patches/*")
    allowed_untracked = {I_DOC_REL, I_SCRIPT_REL} if phase == "owner_review" else set()
    ...
    print("[K] v0.8.4-H artifacts not modified")
    check("K. v0.8.4-H artifacts not modified", not_modified(H_DOC_REL, H_SCRIPT_REL))
    ...
    for letter, marker in zip(letters, REQUIRED_TEXT_MARKERS):
        print(f"[{letter}] doc contains: {marker}")
        check(f"{letter}. doc contains: {marker}", marker in doc_text)

    print("[AL] doc contains no unsafe done-claims")
    ...
    check(
        f"AL. doc contains no unsafe done-claims（found {found_unsafe}）"
        if found_unsafe
        else "AL. doc contains no unsafe done-claims",
        not found_unsafe,
    )
    ...
    print("[AM] readiness script contains no forbidden application/connector/network imports")
    forbidden_imports = [
        m for m in imported_modules
        if any(m == p or m.startswith(p + ".") for p in FORBIDDEN_IMPORT_MODULE_PREFIXES)
    ]
    check(
        f"AM. readiness script contains no forbidden application/connector/network imports（found {forbidden_imports}）"
        if forbidden_imports
        else "AM. readiness script contains no forbidden application/connector/network imports",
        not forbidden_imports,
    )

    print("[AN] readiness script does not reference secrets/.env files")
    forbidden_constants_found = [s for s in string_constants if s in FORBIDDEN_STRING_CONSTANTS]
    check(
        f"AN. readiness script does not reference secrets/.env files（found {forbidden_constants_found}）"
        if forbidden_constants_found or "os" in imported_modules
        else "AN. readiness script does not reference secrets/.env files",
        not forbidden_constants_found and "os" not in imported_modules,
    )

    print("[AO] readiness script only invokes read-only git plumbing as subprocess")
    self_subprocess_targets = re.findall(r'subprocess\.run\(\s*\[\s*"([^"]+)"', self_text)
    check(
        f"AO. readiness script only invokes read-only git plumbing as subprocess（found {self_subprocess_targets}）"
        if any(t != "git" for t in self_subprocess_targets)
        else "AO. readiness script only invokes read-only git plumbing as subprocess",
        all(t == "git" for t in self_subprocess_targets),
    )

    # -----------------------------------------------------------------
    print("[AP] patches/ remains untracked")
    patches_tracked = git_lines(["ls-files", "--", "patches/"])
    check(
        f"AP. patches/ remains untracked（found tracked {patches_tracked}）"
        if patches_tracked
        else "AP. patches/ remains untracked",
        not patches_tracked,
    )
    ...
    check(
        f"AQ. no tag（found {tags_at_head}）" if tags_at_head else "AQ. no tag",
        not tags_at_head,
    )
    ...
    if FAIL:
        print(f"\nXX v0.8.4-I readiness 失敗 {len(FAIL)} 項：")
        for f in FAIL:
            print(f"   - {f}")
        sys.exit(1)
    else:
        print("PASS: v0.8.4-I roadmap amendment final closeout addendum")
        sys.exit(0)

"""v0.9.6-A readiness check: Callback Contract Plan.
...
DOC_REL = "docs/HERMES_CALLBACK_CONTRACT_PLAN_V0_9_6_A.md"
DOC_PATH = REPO_ROOT / DOC_REL

SELF_SCRIPT_REL = "scripts/check_hermes_callback_contract_plan_v0_9_6_a.py"
...
REQUIRED_PHASE_TITLE = "v0.9.6-A Callback Contract Plan"
...
def main() -> None:
    ...
    print("[A] v0.9.6-A plan doc exists")
    check("A. v0.9.6-A plan doc exists", DOC_PATH.is_file())

    print("[B] v0.9.6-A readiness script exists")
    check("B. v0.9.6-A readiness script exists", SELF_SCRIPT_PATH.is_file())

    print("[C] doc contains exact phase title")
    check("C. doc contains exact phase title", REQUIRED_PHASE_TITLE in doc_text)

    print("[D] doc contains all required safety/contract substrings")
    missing_substrings = [s for s in REQUIRED_DOC_SUBSTRINGS if s not in doc_text]
    check(
        f"D. doc contains all required safety/contract substrings（missing {missing_substrings}）"
        if missing_substrings
        else "D. doc contains all required safety/contract substrings",
        not missing_substrings,
    )

    print("[E] doc defines required future Result Message fields")
    missing_fields = [f for f in REQUIRED_RESULT_MESSAGE_FIELDS if f not in doc_text]
    check(
        f"E. doc defines required future Result Message fields（missing {missing_fields}）"
        if missing_fields
        else "E. doc defines required future Result Message fields",
        not missing_fields,
    )

    print("[F] doc lists future sequence v0.9.6-B/C/D/E/R")
    missing_labels = [label for label in REQUIRED_SEQUENCE_LABELS if label not in doc_text]
    check(
        f"F. doc lists future sequence v0.9.6-B/C/D/E/R（missing {missing_labels}）"
        if missing_labels
        else "F. doc lists future sequence v0.9.6-B/C/D/E/R",
        not missing_labels,
    )

    print("[G] doc defines future callback message family")
    check(
        "G. doc defines future callback message family",
        "Future Callback Message Family" in doc_text,
    )

    print("[H] only allowed v0.9.6-A files are staged")
    staged_files = set(git_lines(["diff", "--cached", "--name-only"]))
    unexpected_staged = sorted(staged_files - ALLOWED_STAGED_FILES)
    check(
        f"H. only allowed v0.9.6-A files are staged（unexpected {unexpected_staged}）"
        if unexpected_staged
        else "H. only allowed v0.9.6-A files are staged",
    ...
    print("[O] no forbidden callback/webhook/runtime keywords appear as implementation in this script")
    found_action_keywords = [kw for kw in FORBIDDEN_CONNECTOR_ACTION_KEYWORDS if kw in self_text]
    check(
        f"O. no forbidden callback/webhook/runtime keywords appear as implementation in this script（found {found_action_keywords}）"
        if found_action_keywords
        else "O. no forbidden callback/webhook/runtime keywords appear as implementation in this script",
    ...
    print("[U] cached diff gate: staged diff is empty or exactly the allowed v0.9.6-A files")
    cached_diff_ok = (not staged_files) or (staged_files == ALLOWED_STAGED_FILES)
    check(
        f"U. cached diff gate: staged diff is empty or exactly the allowed v0.9.6-A files（found {sorted(staged_files)}）"
        if not cached_diff_ok
        else "U. cached diff gate: staged diff is empty or exactly the allowed v0.9.6-A files",
        cached_diff_ok,
    )

    total = len(PASS) + len(FAIL)
    print(f"\nv0.9.6-A Callback Contract Plan readiness")
    print(f"合計：{len(PASS)}/{total} 通過")
    if FAIL:
        print(f"\nXX v0.9.6-A readiness 失敗 {len(FAIL)} 項：")
        for f in FAIL:
            print(f"   - {f}")

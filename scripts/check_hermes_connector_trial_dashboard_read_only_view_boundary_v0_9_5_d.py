"""v0.9.5-D readiness check: Connector Trial Dashboard Read-only View Boundary Plan.
...
DOC_REL = "docs/HERMES_CONNECTOR_TRIAL_DASHBOARD_READ_ONLY_VIEW_BOUNDARY_V0_9_5_D.md"
DOC_PATH = REPO_ROOT / DOC_REL

SELF_SCRIPT_REL = "scripts/check_hermes_connector_trial_dashboard_read_only_view_boundary_v0_9_5_d.py"
...
REQUIRED_PHASE_TITLE = "v0.9.5-D Connector Trial Dashboard Read-only View Boundary Plan"
...
REQUIRED_SECTION_HEADINGS = (
    "Future Dashboard Display Principles",
    "Future Allowed Display Categories",
    "Future Forbidden Display Categories",
...
def main() -> None:
    ...
    print("[A] v0.9.5-D boundary doc exists")
    check("A. v0.9.5-D boundary doc exists", DOC_PATH.is_file())

    print("[B] v0.9.5-D readiness script exists")
    check("B. v0.9.5-D readiness script exists", SELF_SCRIPT_PATH.is_file())
    ...
    print("[E] doc defines required section headings (display principles/categories/controls/rendering)")
    ...
    print("[F] doc defines required future UI labels")
    missing_labels = [label for label in REQUIRED_LABELS if label not in doc_text]
    check(
        f"F. doc defines required future UI labels（missing {missing_labels}）"
        if missing_labels
        else "F. doc defines required future UI labels",
        not missing_labels,
    )

    print("[G] only allowed v0.9.5-D files are staged")
    staged_files = set(git_lines(["diff", "--cached", "--name-only"]))
    unexpected_staged = sorted(staged_files - ALLOWED_STAGED_FILES)
    check(
        f"G. only allowed v0.9.5-D files are staged（unexpected {unexpected_staged}）"
        if unexpected_staged
        else "G. only allowed v0.9.5-D files are staged",
    ...
    print("[T] cached diff gate: staged diff is empty or exactly the allowed v0.9.5-D files")
    cached_diff_ok = (not staged_files) or (staged_files == ALLOWED_STAGED_FILES)
    check(
        f"T. cached diff gate: staged diff is empty or exactly the allowed v0.9.5-D files（found {sorted(staged_files)}）"
        if not cached_diff_ok
        else "T. cached diff gate: staged diff is empty or exactly the allowed v0.9.5-D files",
        cached_diff_ok,
    )

    total = len(PASS) + len(FAIL)
    print(f"\nv0.9.5-D Connector Trial Dashboard Read-only View Boundary Plan readiness")
    print(f"合計：{len(PASS)}/{total} 通過")
    if FAIL:
        print(f"\nXX v0.9.5-D readiness 失敗 {len(FAIL)} 項：")
        for f in FAIL:
            print(f"   - {f}")

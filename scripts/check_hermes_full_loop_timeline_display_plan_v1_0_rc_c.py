"""v1.0-RC-C readiness check: Full Loop Timeline Display Plan.
...
DOC_REL = "docs/HERMES_FULL_LOOP_TIMELINE_DISPLAY_PLAN_V1_0_RC_C.md"
DOC_PATH = REPO_ROOT / DOC_REL

SELF_SCRIPT_REL = "scripts/check_hermes_full_loop_timeline_display_plan_v1_0_rc_c.py"
...
REQUIRED_PHASE_TITLE = "v1.0-RC-C Full Loop Timeline Display Plan"

REQUIRED_SECTION_HEADINGS = (
    "Future Display File Recommendations",
    "Future Data Source Boundary",
    "Future Allowed Display Location",
    "Future Display Section Title",
    "Required Display Labels",
    "Future Allowed Top-Level Displayed Fields",
    "Future Allowed Timeline Displayed Fields",
    "Future Forbidden Displayed Fields",
    "Future Display Visual Structure",
    "Future Timeline Status Vocabulary",
    "Future Control Boundary",
    "Future CSS Boundary",
    "Future JavaScript Boundary",
    "Future Dashboard Fail-Closed Display Behavior",
    "Future Route Boundary",
)

REQUIRED_SEQUENCE_LABELS = ("v1.0-RC-D", "v1.0-RC-E", "v1.0-RC-R")
...
def main() -> None:
    ...
    print("[A] v1.0-RC-C plan doc exists")
    check("A. v1.0-RC-C plan doc exists", DOC_PATH.is_file())

    print("[B] v1.0-RC-C readiness script exists")
    check("B. v1.0-RC-C readiness script exists", SELF_SCRIPT_PATH.is_file())
    ...
    print("[E] doc defines required section headings (location/labels/fields/structure/vocab/boundaries/fail-closed/route)")
    ...
    print("[F] doc lists future sequence v1.0-RC-D/E/R")
    missing_labels = [label for label in REQUIRED_SEQUENCE_LABELS if label not in doc_text]
    check(
        f"F. doc lists future sequence v1.0-RC-D/E/R（missing {missing_labels}）"
        if missing_labels
        else "F. doc lists future sequence v1.0-RC-D/E/R",
        not missing_labels,
    )
    ...
    print("[H] only allowed v1.0-RC-C files are staged")
    staged_files = set(git_lines(["diff", "--cached", "--name-only"]))
    unexpected_staged = sorted(staged_files - ALLOWED_STAGED_FILES)
    check(
        f"H. only allowed v1.0-RC-C files are staged（unexpected {unexpected_staged}）"
        if unexpected_staged
        else "H. only allowed v1.0-RC-C files are staged",
    ...
    print("[W] cached diff gate: staged diff is empty or exactly the allowed v1.0-RC-C files")
    cached_diff_ok = (not staged_files) or (staged_files == ALLOWED_STAGED_FILES)
    check(
        f"W. cached diff gate: staged diff is empty or exactly the allowed v1.0-RC-C files（found {sorted(staged_files)}）"
        if not cached_diff_ok
        else "W. cached diff gate: staged diff is empty or exactly the allowed v1.0-RC-C files",
        cached_diff_ok,
    )

    total = len(PASS) + len(FAIL)
    print(f"\nv1.0-RC-C Full Loop Timeline Display Plan readiness")
    print(f"合計：{len(PASS)}/{total} 通過")
    if FAIL:
        print(f"\nXX v1.0-RC-C readiness 失敗 {len(FAIL)} 項：")
        for f in FAIL:
            print(f"   - {f}")

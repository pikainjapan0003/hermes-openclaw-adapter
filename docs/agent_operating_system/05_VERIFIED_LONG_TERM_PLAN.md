...
- 目標與節律：**每 session**＝Phase 0 三源檢查、踩坑即回寫 90；**每 10 個 session 或每月（先到者）**＝跑一次「制度健檢」——90 是否超 300 行需精簡（40 F4）、文件互引是否斷鏈、01 是否被繞過的案例；**每次 phase closeout**＝更新本計劃表的 Phase 狀態行（只改狀態不改結構）；**每季**＝重新驗證 2.3 參考的活性（deprecated 的移除）、重讀 Anthropic/OWASP 是否有新版。
...
- 模糊題與品味判斷（例：v1.0 該包含什麼——Phase 2 必須 Owner 裁決）；產品方向與商業取捨（例：connector 該優先接哪個服務）；「這樣做值不值得」類問題。

...
| R4-03 | P3 | `research/05_COMPACTION_RULE_CROSSWALK_20260723.md:9-18`; current `05_VERIFIED_LONG_TERM_PLAN.md` (`wc -l` at NIGHT-BATCH-13 package-3 start, commit `9da4f78`: 445) | Resolved by NIGHT-BATCH-13 package 3. The crosswalk's ≤440 result is now explicitly labelled as the NIGHT-BATCH-11 package-3 compaction snapshot, not a permanent current-size invariant. The current plan is 445 lines after the authorized onboarding clarification. No rule is missing and the F4 maximum of 500 is not breached. |

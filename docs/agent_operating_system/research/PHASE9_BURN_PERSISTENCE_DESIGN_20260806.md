...
Status: **PLANNING ONLY. 實作授權未給予，Phase 9 稽核寫入授權未給予，執行授權未給予。**

本文記錄 2026-08-06 Owner 就「token 作廢紀錄要寫到哪裡」所做的三項決定，
以及據此產生的設計。本文**不授權**任何寫入、實作或執行。
...
依 `05` §6.18 的逐字裁決，授權句由 Owner 自行產生。系統只保留
`owner_instruction_digest`（SHA-256），不保留原文
（`app/phase9_gate.py:312` 的 `Phase9AuditAuthorizationRecord` 僅有
...
本文是設計紀錄。它**不是**實作授權、寫入授權或執行授權。在 Owner 給出
Phase 9 稽核寫入授權之前，gate 於找不到授權紀錄時**維持 fail-closed 拒絕啟動**
（`app/phase9_gate.py:745`、:764）。
